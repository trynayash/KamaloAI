...
import { BookOpenText, ChevronRight, CircleHelp, Library, MessageSquareText, PanelLeftClose, PanelLeftOpen, Plus, ShieldCheck } from 'lucide-react';
...
export function KamaloMark({ small = false }: { small?: boolean }) {
  return (
    <div className={`flex items-center gap-2.5 ${small ? 'scale-90 origin-left' : ''}`} data-testid="brand-kamalo">
      <div className="relative grid h-9 w-9 place-items-center rounded-xl bg-[hsl(var(--accent))] text-[hsl(var(--sidebar))] shadow-[0_6px_18px_hsl(var(--accent)/.18)]">
        <span className="font-serif text-[24px] font-semibold leading-none">K</span>
      ...
      <div className="leading-none">
        ...
        <div className="mt-1 font-mono text-[8px] tracking-[.22em] text-sidebar-foreground/50">AI COMPANION</div>
      </div>
    </div>
  );
}
...
export function KamaloShell({ children, conversationCount = 0, onNewConversation, mobileOpen = false, onMobileOpenChange }: KamaloShellProps) {
  ...
  return (
    <div className="paper-grain flex min-h-[100dvh] bg-background">
      ...
      />
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[276px] flex-col border-r border-sidebar-border bg-sidebar px-5 py-6 text-sidebar-foreground transition-transform duration-300 md:static md:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-start justify-between">
          <Link href="/" onClick={closeMobile} className="block" data-testid="link-kamalo-home"><KamaloMark /></Link>
          <button onClick={closeMobile} className="rounded-lg p-2 text-sidebar-foreground/55 hover:bg-sidebar-accent hover:text-sidebar-foreground md:hidden" aria-label="Close navigation" data-testid="button-close-sidebar"><PanelLeftClose size={17} /></button>
        </div>
        ...
        <div className="mt-auto space-y-5">
          ...
          <div className="flex items-center gap-3 border-t border-sidebar-border pt-4">
            <div className="grid h-8 w-8 place-items-center rounded-full bg-[hsl(var(--primary))] font-mono text-[11px] text-primary-foreground">KS</div>
      ...
      <main className="min-w-0 flex-1">
        <div className="flex items-center border-b border-border/70 px-4 py-3 md:hidden">
          <button onClick={() => onMobileOpenChange?.(true)} className="rounded-lg p-2 hover:bg-muted" aria-label="Open navigation" data-testid="button-open-navigation"><PanelLeftOpen size={19} /></button>
...
export function SectionLabel({ children }: { children: ReactNode }) {
  return <div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[.19em] text-muted-foreground"><span className="h-1.5 w-1.5 rounded-full bg-[hsl(var(--accent))]" />{children}<ChevronRight size={12} className="opacity-40" /></div>;
}

export function KnowledgeIcon() {
  return <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[hsl(var(--primary)/.1)] text-primary"><Library size={18} /></div>;
}