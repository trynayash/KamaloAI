...
import { getListSupportTicketsQueryKey, useAnalyzeSupportTicket, useListSupportTickets, useUpdateSupportTicket } from '@workspace/api-client-react';
...
function TicketListItem({ ticket, selected, onSelect }: { ticket: SupportTicket; selected: boolean; onSelect: () => void }) {
  return <button onClick={onSelect} className={`w-full border-b border-border/80 px-4 py-4 text-left transition-colors ${selected ? 'bg-primary/[.07]' : 'hover:bg-muted/40'}`} data-testid={`button-admin-ticket-${ticket.id}`}><div className="flex items-center justify-between gap-3"><span className="font-mono text-[9px] font-bold tracking-[.08em] text-primary">{ticket.ticketNumber}</span><span className={`rounded-md px-2 py-1 font-mono text-[8px] uppercase tracking-[.1em] ${tone[ticket.status]}`}>{ticket.status.replace('_', ' ')}</span></div><div className="mt-2 truncate text-[12px] font-bold">{ticket.summary}</div><div className="mt-1 truncate text-[10px] text-muted-foreground">{ticket.category} · {dateTime(ticket.createdAt)}</div></button>;
}
...
function TicketDetail({ ticket, onSaved }: { ticket: SupportTicket; onSaved: (message: string) => void }) {
  ...
  const save = async () => {
    ...
    try {
      await update.mutateAsync({ ticketId: ticket.id, data: { status, assignedTo: assignedTo.trim() || null, resolution: resolution.trim() || null, resolutionSource: mode } });
      await queryClient.invalidateQueries({ queryKey: getListSupportTicketsQueryKey() });
      onSaved(status === 'resolved' ? 'Resolution saved. Email delivery was attempted.' : 'Ticket updated.');
    } catch {
  ...
  return <div className="min-w-0 p-5 sm:p-7" data-testid="panel-admin-ticket-detail">
    <div className="flex flex-wrap items-start justify-between gap-4 border-b border-border pb-5"><div><SectionLabel>Ticket / {ticket.category}</SectionLabel><h2 className="mt-3 text-2xl font-extrabold tracking-[-.045em]">{ticket.summary}</h2><div className="mt-2 flex flex-wrap items-center gap-2 font-mono text-[9px] text-muted-foreground"><span>{ticket.ticketNumber}</span><span>·</span><span>Opened {dateTime(ticket.createdAt)}</span><span>·</span><span>{ticket.contactEmail || 'No contact email'}</span></div></div><span className={`rounded-md px-2.5 py-1.5 font-mono text-[9px] uppercase tracking-[.12em] ${tone[ticket.status]}`}>{ticket.status.replace('_', ' ')}</span></div>
    <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_260px]">
      <div className="min-w-0">
        <div className="rounded-xl border border-border bg-background p-4"><div className="font-mono text-[9px] uppercase tracking-[.14em] text-muted-foreground">Customer issue brief</div><p className="mt-3 whitespace-pre-wrap text-[12px] leading-6">{ticket.details}</p></div>
      ...
      <div className="min-w-0"><label className="block"><span className="mb-2 block font-mono text-[9px] uppercase tracking-[.14em] text-muted-foreground">Status</span><select value={status} onChange={(event) => setStatus(event.target.value as SupportTicketUpdateStatus)} className="h-10 w-full rounded-lg border border-input bg-background px-3 text-[11px] capitalize outline-none focus:border-primary" data-testid="select-ticket-status">{statuses.map((item) => <option key={item} value={item}>{item.replace('_', ' ')}</option>)}</select></label><label className="mt-4 block"><span className="mb-2 block font-mono text-[9px] uppercase tracking-[.14em] text-muted-foreground">Assigned to</span><input value={assignedTo} onChange={(event) => setAssignedTo(event.target.value)} placeholder="Human specialist or KAMALO AI" className="h-10 w-full rounded-lg border border-input bg-background px-3 text-[11px] outline-none focus:border-primary focus:ring-4 focus:ring-primary/10" data-testid="input-ticket-assignee" /></label><label className="mt-4 block"><span className="mb-2 block font-mono text-[9px] uppercase tracking-[.14em] text-muted-foreground">Resolution to send</span><textarea value={resolution} onChange={(event) => setResolution(event.target.value)} maxLength={4000} rows={10} placeholder="Write the verified answer the user should receive." className="w-full resize-y rounded-lg border border-input bg-background px-3 py-3 text-[11px] leading-5 outline-none focus:border-primary focus:ring-4 focus:ring-primary/10" data-testid="input-ticket-resolution" /></label><button onClick={() => void save()} disabled={update.isPending} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-3 text-[11px] font-bold text-primary-foreground hover:-translate-y-0.5 disabled:opacity-50" data-testid="button-save-ticket-resolution"><HiOutlineCheck size={14} />{update.isPending ? 'Saving…' : status === 'resolved' ? 'Save & notify user' : 'Save ticket update'}</button><div className="mt-3 flex items-center gap-2 text-[10px] text-muted-foreground"><HiOutlineEnvelope size={13} />{ticket.emailStatus === 'sent' ? 'Last notification sent' : ticket.emailStatus === 'failed' ? 'Last notification failed' : ticket.contactEmail ? 'Notification available' : 'Add an email to notify'}</div></div>
    </div>
  </div>;
}
...
export function AdminTicketsPage() {
  ...
  return <KamaloShell><div className="mx-auto min-h-[calc(100dvh-57px)] max-w-[1400px] px-4 pb-12 sm:px-6 md:min-h-[100dvh] md:px-9 md:py-8 lg:px-12"><header className="flex flex-col gap-4 border-b border-border/70 pb-7 pt-5 md:flex-row md:items-end md:justify-between md:pt-0"><div><SectionLabel>Temporary test access / Support operations</SectionLabel><h1 className="mt-3 text-[clamp(2rem,4vw,3.25rem)] font-extrabold leading-none tracking-[-.06em]">Ticket control room.</h1><p className="mt-4 max-w-xl text-[13px] leading-6 text-muted-foreground">Review customer context, choose AI or human analysis, and publish a verified resolution. This page is intentionally exposed for testing until authentication is added.</p></div><div className="flex items-center gap-2"><span className="rounded-md border border-[hsl(var(--accent)/.45)] bg-[hsl(var(--accent)/.14)] px-2.5 py-1.5 font-mono text-[9px] uppercase tracking-[.12em] text-[hsl(31_60%_35%)]">Test admin</span><button onClick={() => void ticketsQuery.refetch()} className="grid h-9 w-9 place-items-center rounded-lg border border-border bg-card text-muted-foreground hover:text-primary" aria-label="Refresh tickets" data-testid="button-refresh-tickets"><HiOutlineArrowPath size={15} /></button></div></header>{notice && <div className="mt-4 rounded-lg border border-primary/20 bg-primary/5 px-4 py-3 text-[11px] text-primary animate-rise" role="status" data-testid="status-admin-ticket-notice">{notice}</div>}{ticketsQuery.isLoading ? <div className="mt-7 grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)]"><div className="skeleton h-[460px] rounded-xl" /><div className="skeleton h-[460px] rounded-xl" /></div> : ticketsQuery.isError ? <div className="mt-7 rounded-xl border border-destructive/20 bg-destructive/5 p-10 text-center"><p className="text-[13px] font-semibold">Ticket queue unavailable.</p><button onClick={() => void ticketsQuery.refetch()} className="mt-4 rounded-lg border border-border bg-card px-4 py-2 text-[11px] font-bold" data-testid="button-retry-admin-tickets">Try again</button></div> : tickets.length === 0 ? <div className="mt-7 rounded-xl border border-dashed border-border bg-card/60 p-12 text-center"><HiOutlineInboxStack size={26} className="mx-auto text-muted-foreground" /><h2 className="mt-4 text-xl font-extrabold">Queue is clear.</h2><p className="mt-2 text-[12px] text-muted-foreground">Tickets raised from the chat will appear here.</p></div> : <div className="mt-7 grid min-w-0 gap-6 lg:grid-cols-[300px_minmax(0,1fr)]"><section className="thin-scrollbar min-w-0 overflow-hidden rounded-xl border border-border bg-card shadow-[var(--shadow-sm)]"><div className="flex items-center justify-between border-b border-border bg-muted/40 px-4 py-3"><span className="font-mono text-[9px] uppercase tracking-[.16em] text-muted-foreground">Queue</span><span className="font-mono text-[10px] text-muted-foreground">{tickets.length}</span></div>{tickets.map((ticket) => <TicketListItem key={ticket.id} ticket={ticket} selected={ticket.id === selected?.id} onSelect={() => setSelectedId(ticket.id)} />)}</section><section className="min-w-0 overflow-hidden rounded-xl border border-border bg-card shadow-[var(--shadow-sm)]">{selected && <TicketDetail key={selected.id} ticket={selected} onSaved={showNotice} />}</section></div>}</div></KamaloShell>;
}