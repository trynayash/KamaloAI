...
function TicketListItem({ ticket, selected, onSelect }: { ticket: SupportTicket; selected: boolean; onSelect: () => void }) {
  return <button onClick={onSelect} className={`w-full border-b border-border/80 px-3 py-4 text-left transition-colors sm:px-4 ${selected ? 'bg-primary/[.07]' : 'hover:bg-muted/40'}`} data-testid={`button-admin-ticket-${ticket.id}`}><div className="flex items-start justify-between gap-3"><span className="min-w-0 truncate font-mono text-[9px] font-bold tracking-[.08em] text-primary">{ticket.ticketNumber}</span><span className={`shrink-0 rounded-md px-2 py-1 font-mono text-[8px] uppercase tracking-[.1em] ${tone[ticket.status]}`}>{ticket.status.replace('_', ' ')}</span></div><div className="mt-2 truncate text-[12px] font-bold">{ticket.summary}</div><div className="mt-1 truncate text-[10px] text-muted-foreground">{ticket.category} · {dateTime(ticket.createdAt)}</div></button>;
}
