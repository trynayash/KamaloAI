...
import { Archive, Check, ChevronDown, FilePlus2, Filter, Pencil, RotateCcw, Search, X } from 'lucide-react';
    ...
    <KamaloShell>
      <div className="mx-auto min-h-[calc(100dvh-57px)] max-w-[1320px] px-4 pb-12 sm:px-6 md:min-h-[100dvh] md:px-9 md:py-8 lg:px-12">
        ...
        <div className="mt-7 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative w-full lg:max-w-[460px]"><Search size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search title or article content" className="h-11 w-full rounded-lg border border-input bg-card pl-9 pr-3 text-[12px] outline-none focus:border-primary focus:ring-4 focus:ring-primary/10" data-testid="input-search-articles" /></div>
          <div className="flex flex-wrap items-center gap-2"><div className="flex items-center gap-2 text-muted-foreground"><Filter size={14} /><span className="font-mono text-[10px] uppercase tracking-[.13em]">Filter</span></div><select value={category} onChange={(event) => setCategory(event.target.value)} className="h-10 max-w-[calc(50vw-24px)] rounded-lg border border-input bg-card px-3 text-[11px] outline-none focus:border-primary" data-testid="select-article-category"><option value="">All categories</option>{Array.from(new Set(articles.map((article) => article.category))).sort().map((item) => <option value={item} key={item}>{item}</option>)}</select><div className="relative"><select value={status} onChange={(event) => setStatus(event.target.value as ArticleStatus)} className="h-10 appearance-none rounded-lg border border-input bg-card py-0 pl-3 pr-8 text-[11px] capitalize outline-none focus:border-primary" data-testid="select-article-status"><option value="all">All statuses</option><option value="draft">Draft</option><option value="approved">Approved</option><option value="archived">Archived</option></select><ChevronDown size={13} className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground" /></div>{hasFilters && <button onClick={clearFilters} className="flex h-10 items-center gap-1.5 rounded-lg border border-border px-3 text-[11px] font-semibold text-muted-foreground hover:bg-muted hover:text-foreground" data-testid="button-clear-article-filters"><RotateCcw size={13} /> Reset</button>}</div>
        </div>
