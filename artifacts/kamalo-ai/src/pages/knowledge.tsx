...
import { HiOutlineArchiveBox, HiOutlineCheck, HiOutlineDocumentPlus, HiOutlineFunnel, HiOutlineMagnifyingGlass, HiOutlinePencilSquare, HiOutlineArrowPath, HiOutlineXMark } from '@/components/kamalo-icons';
       ...
       <div className="page-frame min-h-[calc(100dvh-57px)] pb-12 md:min-h-[100dvh] md:pb-16">
         ...
         <div className="mt-7 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="relative w-full lg:max-w-[460px]"><HiOutlineMagnifyingGlass size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search title or article content" className="h-11 w-full rounded-lg border border-input bg-card pl-9 pr-3 text-[12px] outline-none focus:border-primary focus:ring-4 focus:ring-primary/10" data-testid="input-search-articles" /></div>
           <div className="flex flex-wrap items-center gap-2"><div className="flex items-center gap-2 text-muted-foreground"><HiOutlineFunnel size={14} /><span className="font-mono text-[10px] uppercase tracking-[.13em]">Filter</span></div><KamaloSelect value={category || '__all_categories__'} onValueChange={(value) => setCategory(value === '__all_categories__' ? '' : value)} ariaLabel="Filter by article category" testId="select-article-category" className="max-w-[calc(50vw-24px)] bg-card" options={[{ value: '__all_categories__', label: 'All categories' }, ...Array.from(new Set(articles.map((article) => article.category))).sort().map((item) => ({ value: item, label: item }))]} /><KamaloSelect value={status} onValueChange={(value) => setStatus(value as ArticleStatus)} ariaLabel="Filter by article status" testId="select-article-status" className="w-auto min-w-[9rem] bg-card" options={[{ value: 'all', label: 'All statuses' }, { value: 'draft', label: 'Draft' }, { value: 'approved', label: 'Approved' }, { value: 'archived', label: 'Archived' }]} />{hasFilters && <button onClick={clearFilters} className="flex h-10 items-center gap-1.5 rounded-lg border border-border px-3 text-[11px] font-semibold text-muted-foreground hover:bg-muted hover:text-foreground" data-testid="button-clear-article-filters"><HiOutlineArrowPath size={13} /> Reset</button>}</div>
        </div>
