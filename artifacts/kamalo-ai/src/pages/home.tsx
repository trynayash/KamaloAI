      ...
      <div className="max-w-md">
        ...
        {showQuickPrompts && <div className="mt-7 grid gap-2 sm:grid-cols-3">
           {quickPrompts.map((prompt) => (
             <button key={prompt.label} onClick={() => onPrompt(prompt.text)} className="button-secondary rounded-lg border border-border/80 bg-card/60 px-3 py-3 text-left" data-testid={`button-prompt-${prompt.label.toLowerCase()}`}>
          ...
          {assistant && (
            <>
              <span className="mx-1 opacity-40">·</span>
               <button onClick={() => onCopy(displayContent)} className="icon-button rounded-md p-1.5" aria-label="Copy assistant response" data-testid={`button-copy-message-${message.id}`}><HiOutlineClipboardDocument size={13} /></button>
               <button onClick={() => onFeedback(message, 'helpful')} className={`icon-button rounded-md p-1.5 hover:text-primary ${message.feedback === 'helpful' ? 'text-primary' : ''}`} aria-label="Mark response helpful" data-testid={`button-helpful-${message.id}`}><HiOutlineHandThumbUp size={13} /></button>
               <button onClick={() => onFeedback(message, 'not_helpful')} className={`icon-button rounded-md p-1.5 hover:text-destructive ${message.feedback === 'not_helpful' ? 'text-destructive' : ''}`} aria-label="Mark response not helpful" data-testid={`button-not-helpful-${message.id}`}><HiOutlineHandThumbDown size={13} /></button>
               {onRetry && <button onClick={onRetry} className="icon-button rounded-md p-1.5" aria-label="Retry last text prompt" data-testid={`button-retry-message-${message.id}`}><HiOutlineArrowPath size={13} /></button>}
            </>
          )}
        </div>
      </div>
    ...
    <KamaloShell conversationCount={conversations.length} onNewConversation={startNewConversation} lockChrome>
       <div ref={messagesScrollRef} onScroll={handleWorkspaceScroll} className="chat-workspace mx-auto flex min-h-0 w-full max-w-[1320px] flex-1 flex-col px-4 pb-40 sm:px-6 sm:pb-36 md:px-9 md:py-7 lg:px-12" onPointerDown={markUserActivity} onKeyDown={markUserActivity}>
         {selectedId && <header className="flex items-center justify-between border-b border-border/70 py-4 md:border-0 md:py-0">
            ...
            <button onClick={clearCurrent} disabled={!selectedId || deleteConversation.isPending} className="button-secondary hidden items-center gap-2 rounded-md border border-border bg-card/60 px-3 py-2 text-[11px] font-semibold text-muted-foreground hover:border-destructive/30 hover:bg-destructive/5 hover:text-destructive disabled:opacity-40 sm:flex" data-testid="button-clear-conversation"><HiOutlineBackspace size={14} /> Clear</button>
         </header>}
                    ...
                    <div className="flex items-center justify-between gap-2 px-2 pb-0.5">
                      <div className="flex min-w-0 items-center gap-2">
                        <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,.jpg,.jpeg,.png" className="sr-only" onChange={(event) => { chooseImage(event.target.files?.[0]); event.target.value = ''; }} data-testid="input-chat-attachment" />
                         <button type="button" onClick={() => { inputModeRef.current = 'voice'; speech.toggle(); }} disabled={isSending || speech.isSupported === false} className={`button-secondary inline-flex h-10 shrink-0 items-center gap-1.5 rounded-lg border px-2.5 text-[10px] font-semibold disabled:opacity-40 ${speech.isListening ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground'}`} aria-label={speech.isListening ? 'Stop voice input' : 'Start voice input'} data-testid="button-voice-input">{speech.isListening ? <HiOutlineStop size={14} /> : <HiOutlineMicrophone size={14} />} <span className="hidden sm:inline">{speech.isListening ? 'Stop' : 'Voice'}</span></button>
                         <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isSending} className="button-secondary inline-flex h-10 shrink-0 items-center gap-1.5 rounded-lg border border-border px-2.5 text-[10px] font-semibold text-muted-foreground disabled:opacity-40" aria-label="Attach a JPG or PNG image" aria-describedby="attachment-help" data-testid="button-attach-image"><HiOutlinePaperClip size={14} /> <span className="hidden sm:inline">Attach image</span></button>
                       ...
                       <button onClick={() => void sendMessage()} disabled={(!input.trim() && !pendingImage) || isSending} className="button-primary grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-primary text-primary-foreground disabled:opacity-35" aria-label={isSending ? 'Sending message' : 'Send message'} data-testid="button-send-message"><HiOutlinePaperAirplane size={15} /></button>
                    </div>
                 </div>
