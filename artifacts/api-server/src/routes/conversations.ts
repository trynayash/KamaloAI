...
router.post("/conversations/:conversationId/messages", async (req, res): Promise<void> => {
  ...
  recordSupportEvent(req.log, {
    ...
    analysisSource: prepared.questionAnalysis.source,
    questionType: prepared.questionAnalysis.questionType,
    retrievalQuery: prepared.questionAnalysis.retrievalQuery,
  });
  recordSupportEvent(req.log, {
  ...
  try {
    if (attachment && !content) {
      ...
      if (prepared.groundedFact) {
        fullResponse = prepared.groundedFact;
    ...
    const kbFallback = knowledgeBackedFallback({
      question: content,
      groundedFact: prepared.groundedFact,
      retrieved: prepared.retrieved,
    });
    if (kbFallback) {
      fullResponse = kbFallback;
  ...
  if (
    ...
    const intentCorrected = selectGroundedAnswer(content, prepared.retrieved, prepared.groundedFact);
    if (intentCorrected) {
      candidateResponse = intentCorrected;
      responseOutcome = "grounding_fallback";
    }
  }
  ...
  req.log.info({
    ...
    requestId: supportContext.requestId,
    model: prepared.answerModel,
    routingModel: prepared.questionAnalysis.source,
