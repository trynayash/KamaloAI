...
import { UNKNOWN_KAMALO_RESPONSE } from "../src/lib/support-responses";
import {
  conversationsTable,
  db,
...
test("does not retrieve an unrelated article from the KAMALO brand name alone", () => {
  const unrelated = rankKnowledgeArticles("What is KAMALO's cricket sponsorship policy?", representativeKnowledgeFixtures);
...
test("sanitizes and caps content produced by a streamed provider", async () => {
  ...
  try {
    ...
    assert.equal(result.content.endsWith("â€¦"), true);
...
test("returns safe fallbacks for prompt extraction, unknown questions, and provider failures", async () => {
  ...
  assert.match(promptResult.content, /can(?:not|â€™t) reveal internal instructions/i);
  ...
  assert.equal(adjacentResult.content, UNKNOWN_KAMALO_RESPONSE);
  ...
  try {
    ...
    assert.equal(providerResult.content, "Iâ€™m having trouble responding right now. Please try again.");
...
test("returns a knowledge error instead of an unknown answer when retrieval is unavailable", async () => {
  ...
  try {
    ...
    assert.equal(response.status, 200);
    const result = await streamResult(response);
    assert.equal(result.content, "Iâ€™m having trouble responding right now. Please try again.");
...
test("rejects unsupported live and numeric provider claims at the route boundary", async () => {
  ...
  try {
    ...
    assert.equal(result.content, UNKNOWN_KAMALO_RESPONSE);
  } finally {
    llmProvider.stream = originalStream;
  }
});
